...
var createApache = function(domain, documentRoot) {
	var siteConf = domain+".conf";
	var siteConfFull = paths.apache+siteConf;

	fs.writeFileSync(siteConfFull, apacheModel(domain, documentRoot));
...
var removeHosts = function(domain) {
	fs.readFile('/etc/hosts', 'utf-8', function (err, data) {
		...
		var content = data.replace(new RegExp('127\.0\.1\.1 '+domain+' www.'+domain + '\n', 'g'), '');
